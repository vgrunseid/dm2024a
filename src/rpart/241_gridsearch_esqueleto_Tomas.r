...
PARAM$semillas <- c(800011,800029,800033,800039,800051)
...
archivo_salida <- "./exp/HT2020/gridsearch_tomas.txt"
...
print(vmax_depth)
          for (vmin_split in c(650)) {
           print (vmin_split)
           for(vmin_bucket in(325)) {   
            #           for(vmin_bucket in(c(as.integer(vmin_split/2),as.integer(vmin_split/3),as.integer(vmin_split/4)))) {   
             ...
             tb_grid_search <- rbindlist( 
               list( tb_grid_search, 
                     list(  vmax_depth,vmin_bucket, vmin_split,vcp, ganancia_promedio) ) )
             
             
             
             
             #                 for(vmin_bucket in(c(vmin_split/2,vmin_split/3,vmin_split/4))) {
#  for(vmin_bucket in(c(as.integer(vmin_split/2),as.integer(vmin_split/3),as.integer(vmin_split/4)))) {
